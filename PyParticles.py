...
def addVectors(v1,v2):
    ...
    (angle1, length1), (angle2, length2) = v1, v2
...
class Particle:
    ...
    def __init__(self, id, point, size, mass=1):
        (x, y) = point
        self.id = id
        self.x = x
        self.y = y
...
class Environment:
    ...
    def assignMaxBallSize(self,max_size):
        self.max_size = max_size
        self.xs = list(range(0, self.width + 1, max_size*2))
        self.ys = list(range(0, self.height + 1, max_size*2))
        self.row_len = len(self.xs)
        self.col_len = len(self.ys)
        pass
    ...
    def addParticles(self, n=1, **kargs):
        ...
        for i in range(n):
            ...
            particle = Particle(len(self.particles), (x, y), size, mass)
    ...
    def update(self):
        ...
        box_size = self.max_size
        
        boxes = [[] for _ in range(self.row_len * self.col_len)]
        ...
        for i, particle in enumerate(self.particles, 1):
            ...
            last_x = 0
            for xi in range(1,self.row_len):
                last_y = 0
                for yi in range(1,self.col_len):
                    if grouped:
                        seen_since += 1
                    if seen_since > (self.row_len + 1):
                        grouped = True
                        break
                    if seen_in >= 4: 
                        grouped = True
                        break
                    if ((self.xs[xi-1] - box_size/2) <= particle.x <= (self.xs[xi] + box_size/2)) and ((self.ys[yi-1] - box_size/2) <= particle.y <= (self.ys[yi] + box_size/2)):
                        seen_in += 1 
                        seen_since = 0
                        boxes[((yi -1) * self.row_len) + (xi - 1)].append(i)
                        
        for box in boxes:
            for i, particle_id in enumerate(box, 1):
                particle = self.particles[particle_id - 1]
                for particle2_id in box[i:]:
                    particle2 = self.particles[particle2_id - 1]
